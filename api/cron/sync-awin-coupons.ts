...
import { syncAwinCoupons } from '../../lib/affiliate/awinCouponsSync';
...
export default async function handler(req: any, res: any) {
  ...
  // 1. Verificação de Segurança via CRON_SECRET_TOKEN / CRON_SECRET / Admin manual
  ...
  if (!isManualAdmin && expectedToken) {
    ...
    if (!providedSecret || (providedSecret !== expectedToken && providedSecret !== process.env.CRON_SECRET)) {
      console.warn('[AWIN COUPONS CRON] Tentativa não autorizada de disparo do sync de cupons.');
  ...
  console.log(`⏰ [AWIN COUPONS CRON] Sincronização disparada em ${new Date().toISOString()}`);
  ...
  try {
    ...
    console.error(`❌ [AWIN COUPONS CRON] Erro ao sincronizar cupons:`, err.message);
    return res.status(500).json({
      success: false,
      error: err.message || 'Erro interno ao sincronizar cupons da Awin.',
    });
  }
}
