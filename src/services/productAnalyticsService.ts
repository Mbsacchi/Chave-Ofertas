...
export async function incrementProductClick(productId: string): Promise<void> {
  ...
  try {
    // 1. Tenta invocar a função RPC atômica no Supabase
    const { error: rpcError } = await supabase.rpc('increment_product_clicks', {
      target_product_id: productId,
    });

    if (rpcError) {
      // Fallback: busca o valor atual e incrementa via update direto
    ...
    // 2. Beacon assíncrono para a rota serverless /api/click com keepalive
    try {
    ...
    console.warn('[Analytics] Erro ao incrementar clique do produto:', err.message);
  }
}
